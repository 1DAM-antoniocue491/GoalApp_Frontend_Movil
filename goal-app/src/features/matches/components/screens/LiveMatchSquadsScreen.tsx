 ...
 * Vista solo lectura de plantillas en partido en vivo.
 * No permite seleccionar, modificar ni guardar jugadores.
 */
...
function ReadOnlySquads({ title }: { title: string }) {
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: Colors.bg.base }}><StatusBar barStyle="light-content" />
    ...
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }}>
      ...
      <View style={{ backgroundColor: Colors.bg.surface1, borderRadius: 22, padding: 16, borderWidth: 1, borderColor: Colors.bg.surface2 }}><Text style={{ color: Colors.text.primary, fontSize: 28, fontWeight: '900' }}>{counts.total}</Text><Text style={{ color: Colors.text.secondary, marginTop: 4 }}>Jugadores convocados · {counts.titulares} titulares · {counts.suplentes} suplentes</Text></View>
...
export function LiveMatchSquadsScreen() {
  return <ReadOnlySquads title="Plantillas" />;
}
