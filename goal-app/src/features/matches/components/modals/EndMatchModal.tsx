...
function EndMatchModalComponent({ visible, match, onConfirm, onCancel, submitting = false }: EndMatchModalProps) {
  ...
  const canConfirm = Number(mvpId) > 0 && Number.isFinite(parsedScore) && parsedScore >= 1 && parsedScore <= 10;
      ...
      <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.65)' }}>
        ...
        <View style={{ backgroundColor: Colors.bg.surface1, borderTopLeftRadius: 30, borderTopRightRadius: 30, padding: 22, paddingBottom: 40, maxHeight: '88%' }}>
          <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <TextInput value={mvpScore} onChangeText={setMvpScore} keyboardType="decimal-pad" placeholder="1 - 10" placeholderTextColor={Colors.text.disabled} style={{ height: 52, borderRadius: 16, backgroundColor: Colors.bg.base, color: Colors.text.primary, paddingHorizontal: 16, borderWidth: 1, borderColor: Colors.bg.surface2, fontSize: 16 }} />
