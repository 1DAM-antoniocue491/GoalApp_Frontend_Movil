...
interface GoalEventModalProps { visible: boolean; match: LiveMatchContext | null; onConfirm: (data: GoalEventData) => void; onCancel: () => void; }
...
function GoalEventModalComponent({ visible, match, onConfirm, onCancel }: GoalEventModalProps) {
  ...
  return (
    <Modal transparent visible={visible} animationType="slide" statusBarTranslucent onRequestClose={onCancel}>
      <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.65)' }}><Pressable style={{ flex: 1 }} onPress={onCancel} />
